...
| 话题/Thread 管理 | 每条消息在飞书子话题中回复；`/thread` 列出活跃话题；`/thread close <id>` 强制关闭；`/done` 标记完成 |
| 访问控制（ACL） | 基于角色的权限控制：Admin / Owner / Collaborator；内置申请与审批流程；支持 `require_at_mention` 模式 |
| 元命令 | `/new` `/stop` `/help` `/status` `/project` `/task` `/remember` `/skill` `/thread` `/done` `/whoami` `/acl` |
...
| `/whoami` | 显示您的飞书 `open_id` 和当前角色 |
| `/thread` | 列出当前聊天中所有活跃话题 |
| `/thread close <id>` | 通过短 ID 强制关闭指定话题 |
| `/done` | 标记当前任务完成并关闭对应话题 |
...
| `streaming_enabled` | `true` | 启用 CardKit 流式进度更新 |
| `require_at_mention` | `false` | 仅处理在群聊中 @bot 的消息 |
| `max_active_threads_per_chat` | `100` | 每个聊天中最多同时活跃的话题数 |
| `admin_users` | `[]` | 管理员 `open_id` 列表；管理员拥有完全访问权限并可审批 Owner 申请 |
| `log_level` | `"INFO"` | 日志级别 |
...
├── state.json           # 会话状态（actual_id, 活跃项目, 动态绑定, 话题记录）
├── nextme.db            # ACL 数据库（SQLite；用户、角色、待审批申请）
...
- **Phase 3 ✅** — 访问控制（ACL）角色体系 + 申请审批流程、话题/Thread 管理、`require_at_mention`、CardKit 流式进度
- **Phase 4** — 配置热重载、Slack / 钉钉适配器、多 Agent 编排
