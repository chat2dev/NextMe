...
| end | 结束时间 | start + 1小时 |
| attendees | 参与人(@mentions)段落中每行的 open_id **加上**"预定人 open_id"字段的值，去重后逗号拼接（排除 Bot 自身的 @mention，仅保留真实用户） | 仅预定人自身 |
| room | 会议室关键词，如"极光"；若用户未提及，留空 | ""（空，仅创建线上会议）|

## 步骤 2：确定脚本路径

用 `find` 在项目目录及 worktree 中查找脚本（处理开发阶段 worktree 与生产环境路径差异）：

```bash
SCRIPT=$(find "$(pwd)" "$(pwd)"/.worktrees/*/scripts -name feishu_book_meeting.py 2>/dev/null | head -1)
if [ -z "$SCRIPT" ]; then
  echo '{"ok": false, "error": "找不到预定脚本 feishu_book_meeting.py，请确认已合入主分支或在 worktree 中开发"}'
  exit 1
fi
```
