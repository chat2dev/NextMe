...
class Settings(BaseModel):
    ...
    """
    scheduler_overdue_grace_minutes: int = 5
    """定时任务过期宽限期（分钟）。

    当 bot 重启或调度器暂停后，已超期的定时任务按以下规则处理：

    - 过期时长 ≤ 宽限期：照常执行，并在回复中附加过期标记（⏰ 延迟 N 分钟）。
    - 过期时长 > 宽限期：跳过本次执行，直接计算下一次触发时间。
    - 设为 ``-1``：禁用跳过，所有过期任务均会执行（附过期标记）。
    - 设为 ``0``：零宽限，任何超期都直接跳过。
    """
